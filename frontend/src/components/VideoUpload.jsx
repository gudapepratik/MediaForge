...
import { getFileHash } from "../utils/upload.utils";
...
function VideoUpload() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const checksum = await getFileHash(file);

      const {data: {data}} = await axios.post(`${config.BACKEND_ENDPOINT}/api/videos/create-multipart-upload-request`, 
        {
          fileName: file.name,
          fileSize: file.size,
      ...
      if(data.isExist) {
        alert("File already exists, you can resume the upload..")
        navigate('/uploads/?new=false&id=')
      } else{
        navigate('/uploads/?new=true')
      }
